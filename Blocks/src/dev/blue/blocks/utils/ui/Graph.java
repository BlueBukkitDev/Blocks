...
public class Graph extends UIObject {
	...
	public Graph(App app, String title, Color titleColor, Font titleFont, int x, int y, int width, int height, int cushion, Color lineColor, Double[] values, double peak) {
		this.app = app;
		this.title = title;
	...
	private void setupDefaultInfoPanel() {
		...
		if (total > 0) {
			this.infoPanel
					.addLine(
							new TextBit[] {
									new TextBit(app, new Color(255, 174, 0), new Font("Helvetica", 0, 18),
											"Lessons Complete: " + complete + "/" + total + " ("
													+ (int) Math.floor(complete / total * 100.0D) + "%)",
											null, null) });
		} else {
			this.infoPanel.addLine(new TextBit[] { new TextBit(app, new Color(255, 174, 0), new Font("Helvetica", 0, 18),
		...
		if (status.equalsIgnoreCase("Behind!")) {
			this.infoPanel.addLine(new TextBit[] {
					new TextBit(app, new Color(220, 0, 0), new Font("Helvetica", 0, 18), status, null, null) });
		} else if (status.equalsIgnoreCase("Ahead!")) {
			this.infoPanel.addLine(new TextBit[] {
					new TextBit(app, new Color(0, 220, 0), new Font("Helvetica", 0, 18), status, null, null) });
		} else if (status.equalsIgnoreCase("On Schedule!")) {
			this.infoPanel.addLine(new TextBit[] {
					new TextBit(app, new Color(255, 174, 0), new Font("Helvetica", 0, 18), status, null, null) });
		}
	}
